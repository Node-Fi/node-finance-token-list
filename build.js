...
const TOKEN_LIST = {
  ...
  43113: [
    "https://raw.githubusercontent.com/sushiswap/list/master/lists/token-lists/default-token-list/tokens/fuji.json",
    "https://raw.githubusercontent.com/pangolindex/tokenlists/main/pangolin.tokenlist.json",
  ],
  // celo
  42220: [
...
async function isTokenFresh(erc20Contract, latestBlock, lookBackBlocks) {
  ...
  for (
    let fromBlock = latestBlock - blockIncrement;
...
async function generate(chainId) {
  ...
  for (tokenListUrl of tokenListUrls) {
    ...
    console.info(
      `chain ${chainId} fetched ${rawTokens.length} tokens from ${tokenListUrl}`
    );
  }
...
async function getTokenInfos({ web3, multicall, tokenAddrs, chainId }) {
  const calls = [];
  ...
  const res = await multicall.methods
    .aggregate(calls)
    .call()
...
async function tronUniV1Scan() {
  ...
  const multicall = new web3.eth.Contract(
    TronMulticallABI,
    "0xb8F050B6745510b005E0dc941FcEc5614027d5B7"
  );
  const factory = new web3.eth.Contract(
    IUniswapV1FactoryABI,
    "0xeEd9e56a5CdDaA15eF0C42984884a8AFCf1BdEbb"
  );
  ...
  const tokens = await Promise.all(
    tokenCalls.map((tokenCall) => {
      return multicall.methods
        .aggregate(tokenCall)
        .call()
        .then(({ returnData }) =>
          returnData.map((data) =>
            web3.eth.abi.decodeParameter("address", data)
          )
        );
    })
  ).then((arr) => arr.flat());
  ...
  const exchanges = await Promise.all(
    exchangeCalls.map((exchangeCall) => {
      return multicall.methods
        .aggregate(exchangeCall)
        .call()
        .then(({ returnData }) =>
          returnData.map((data) =>
            web3.eth.abi.decodeParameter("address", data)
  ...
  const balances = await Promise.all(
    balanceCalls.map((balanceCall) => {
      return multicall.methods
        .aggregate(balanceCall)
        .call()
        .then(({ returnData }) =>
          returnData.map((data) =>
  ...
  return await getTokenInfos({
    web3,
    multicall,
    tokenAddrs: freshTokens,
    chainId: 728126428,
  });
}
